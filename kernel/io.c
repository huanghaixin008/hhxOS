...
static u8 read_byte_from_kb_buf();
...
void keyboard_read(TTY* p_tty) {
	...
	if(kb_in.count > 0){
		...
		scan_code = read_byte_from_kb_buf();
		...
		if (scan_code == 0xE1) {
			...
			int is_pausebreak = 1;
			for(i=1;i<6;i++){
				if (read_byte_from_kb_buf() != pausebrk_scode[i]) {
					is_pausebreak = 0;
					break;
				}
			}
			...
			/* PrintScreen 被按下 */
			if (scan_code == 0x2A) {
				if (read_byte_from_kb_buf() == 0xE0) {
					if (read_byte_from_kb_buf() == 0x37) {
						key = PRINTSCREEN;
						make = 1;
					}
				}
